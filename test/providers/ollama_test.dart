...
void main() {
  group('OllamaTextClient', () {
    ...
    test(
      'prepends system, includes history, json format and temperature',
      () async {
        late http.Request seen;
        final client = MockClient((request) async {
          seen = request;
    ...
    test(
      'derives the endpoint from the credentials base url when set',
      () async {
        late http.Request seen;
        final client = MockClient((request) async {
          seen = request;
        ...
        final creds = ProviderCredentials(
          apiKey: '',
          baseUrl: Uri.parse('http://ollama.test:1234'),
        );
        final generator = OllamaTextClient(
          credentials: creds,
          httpClient: client,
    ...
    test('honors an explicit endpoint override over the base url', () async {
      ...
      final creds = ProviderCredentials(
        apiKey: '',
        baseUrl: Uri.parse('http://ignored.test'),
      );
      final generator = OllamaTextClient(
        credentials: creds,
        httpClient: client,
